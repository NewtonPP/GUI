...
const Fingerprints = ({ atoms = [] }) => {
    ...
    setFingerprintsArray((prev) => {
      const updatedArray = prev[atom].map((item, i) => {
        if (i === index) {
          ...
          updatedAlphak[alphakIndex] = alphakValue;
  ...
  const renderParameterInputs = (atom, index, params, fingerprintType) => (
    <>
      {params.map((param) => (
        <div key={param} className="flex items-center mt-2">
          <label className="mr-2 text-base font-semibold">{param}:</label>
          <input
            className="h-4 w-40 border border-gray-300 py-4 px-2 rounded-md"
            onChange={(e) =>
  ...
  return (
    <div className="container mx-auto p-6">
            ...
            <div key={index} className="flex flex-col sm:flex-row items-center gap-4 mt-4">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => {
                    handleFingerprintChange(e.target.value, atom, index);
                  ...
                  <div className="p-2">
                    <select
                      onChange={(e) => {
                        handleParameterChange({
                          atom: atom,
                          index: index,
                          fingerprinttype: e.target.value,
                        });
                    ...
                    >
                      ...
                      {TwoElementsFingerprintValues.map((TwoElementsValue) => (
                        <option key={TwoElementsValue} value={TwoElementsValue}>
                          {TwoElementsValue}
                        </option>
                      ))}
                    </select>
                  ...
                  <div className="p-2">
                    ...
                    >
                      ...
                      {ThreeElementsFingerprintValues.map((ThreeElementsValue) => (
                        <option key={ThreeElementsValue} value={ThreeElementsValue}>
                          {ThreeElementsValue}
                        </option>
                      ))}
                    </select>
                ...
                {SplittedObj[atom][index] === 2 ? (
                  <div>
                    {renderParameterInputs(atom, index, ["re", "rc", "dr", "o", "n"], FingerPrinttypeChange)}
                    {item.params.alpha && (
                      <div className="mt-4">
                        <label className="font-medium text-gray-700">Alpha Array:</label>
                ...
                ) : SplittedObj[atom][index] === 3 || SplittedObj[atom][index] === 4 ? (
                  <div>
                    {renderParameterInputs(atom, index, ["re", "rc", "dr", "m", "k"], FingerPrinttypeChange)}
                    {item.params.alphak && (
                      <div className="mt-4">
                        <label className="font-medium text-gray-700">Alphak Array:</label>
      ...
      <div>
        {atoms.length > 0 && <NetsPerElement atoms={atoms} fingerprints={fingerprintsArray} />}
      </div>
    </div>
  );
};
